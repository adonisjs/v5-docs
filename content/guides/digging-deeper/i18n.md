The `@adonisjs/i18n` is an official package to support [Internationalization and localization](https://en.wikipedia.org/wiki/Internationalization_and_localization) to your AdonisJS applications.
...
- Helpers to perform language-sensitive formatting for a date, currency, names, and so on.
...
You can configure the `defaultLocale` within the config file.
...
:::note
The locale for which you have defined the fallback should be part of the `supportedLocales` array. We will add it to the array if not specified explicitly.
:::
...
The `provideValidatorMessages` flag will hook into the [validator](../validator/custom-messages.md) lifecycle and provides validation messages from the translation files.
